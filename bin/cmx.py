...
from src.src.consts import *

def help():
    print(CMX_HELP)
def clear():
    with open('src/src/trade_journal.json', 'r') as f:
        j = json.load(f)
    ...
    with open('src/src/trade_journal.json', "w") as f:
...
def settings():
    ...
    choice = int(input(SETTINGS_MENU.format(
        sybmol=c['symbol'],
        interval=c['interval'],
        amount=c['amountBuy'],
        rsi=c['RSI'],
    ...
    match choice:
        case 1:
            new_value = input('Send new pair name (BTCUSD, SOLUSDT, ETHUSDT and etc.) [BTCUSDT]\n>>> ')
            change_config('symbol', new_value if new_value != '' else 'BTCUSDT')
            print('Changes saved!')
            settings()
        case 2:
            new_value = input('Send new graph interval (1,5,10,15,30,60,120,240,360,720) [5]\n>>> ')
            if new_value == '':
            ...
            settings()
        case 3:
            new_value = input('Send new order price (min. 5. Recommended 10-15% of your balance) [5]\n>>> ')
            if new_value == '':
        ...
        case 4:
            ...
            settings()
        case 5:
            new_value = input('Send new value for buy step (Recommended 0.3-1% of pair price) [750]\n>>> ') 
            if new_value == '':
                change_config('stepBuy', 750)
            else:
                change_config('stepBuy', int(new_value))
            new_value = input('Send new value for sell step (Recommended 0.3-1% of pair price) [650]\n>>> ') 
            if new_value == '':
            ...
            settings()
        case 6:
            new_value = input('Turn on sending notifies to telegram [y/n](Recommended turn on!) [y]\n>>> ') 
            if new_value == "y": new_value = True
            elif new_value == 'n': new_value = False
